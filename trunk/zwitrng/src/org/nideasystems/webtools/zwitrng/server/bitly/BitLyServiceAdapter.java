	...
	public Map<String, String> shortLinks(List<String> links) throws Exception{
		...
		for (String link: links) {
			...
			if ( !link.startsWith("http://bit.ly")) {
				...
				String url = "http://api.bit.ly/shorten?version=2.0.1&history=1&longUrl="+encodedURL+"&login="+bitUser+"&apiKey="+bitKey;
				...
				try {
					...
					shortedUrl = jObj.getJSONObject("results").getJSONObject(link).getString("shortUrl");
		  ...
		  /*String url = new HttpClient(bitUser, bitPass).get("http://api.bit.ly/expand?version=2.0.1&apiKey=
				  " + apiKey &hash=" + hash, true).asJSONObject()
				                      .getJSONObject
