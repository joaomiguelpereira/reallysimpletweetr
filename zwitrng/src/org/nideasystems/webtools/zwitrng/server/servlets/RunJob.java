...
public class RunJob extends AbstractHttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			throw new ServletException("Please login <a href=\""+userService.createLoginURL(req.getRequestURI()+"\">login</a>"));
		}
		...
		if (req.getParameter("name").equals("followUsers")) {
			...
			try {
				List<PersonaDO> personas = getBusinessHelper().getPersonaDao().findAllPersonas(loggedUser.getEmail());
				
				for (PersonaDO persona: personas ) {
					outBuffer.append(createDiv("Running job for persona: "+persona.getName()));
