...
public class PersonaPojo extends AbstractPojo {
	...
	public PersonaDTOList getAllPersonas(String email) throws Exception {
		...
		for (PersonaDO persona : personas) {
			...
			/*
			 * User twitterUser = null; TwitterAccountDTO
			 * authorizedTwitterAccount = null;
			 * 
			 ...
			 * businessHelper.
			 * getTwitterPojo().getAuthenticatedUser(authorizedTwitterAccount);
			 * //twitterUser =
			 * TwitterServiceAdapter.get().getUserInfo(authorizedTwitterAccount
			 * ); } catch (Exception e) { //No prob, just mean that the user has
			 * to authenticate e.printStackTrace(); }
			 * 
			 * }
			 * 
			 ...
			 * TwitterServiceAdapter.get().getRateLimits
			 * (authorizedTwitterAccount);
			 * 
			 ...
			 * rateLimisDo
			 * .getRateLimitLimit()!=null&&rateLimisDo.getRateLimitRemaining
			 * ()!=null&&rateLimisDo.getRateLimitReset()!=null) {
			 ...
			 * ,authorizedTwitterAccount));
			 * authorizedTwitterAccount.setNewFriends
			 * (getNewFriendsCount(persona,authorizedTwitterAccount));
			 * authorizedTwitterAccount
			 * .setNewBlocking(getNewBlockingCount(persona
			 * ,authorizedTwitterAccount));
			 * 
			 ...
			 * 
			 * authorizedTwitterAccount.setFollowBackQueueSize(persona.
			 * getTwitterAccount
			 * ().getAutoFollowBackIdsQueue()!=null?persona.getTwitterAccount
			 * ().getAutoFollowBackIdsQueue().size():0);
			 * authorizedTwitterAccount
			 * .setIgnoreUsersListSize(persona.getTwitterAccount
			 * ().getIgnoreUsersIds
			 * ()!=null?persona.getTwitterAccount().getIgnoreUsersIds
			 * ().size():0);
			 * authorizedTwitterAccount.setAutoUnfollowQueueSize(persona
			 * .getTwitterAccount
			 * ().getAutoUnFollowBackIdsQueue()!=null?persona.getTwitterAccount
			 * ().getAutoUnFollowBackIdsQueue().size():0);
			 * 
			 * 
			 * 
			 * 
	...
	private PersonaDTO getPersonaDto(PersonaDO persona) throws Exception {
		...
		if (twitterUser != null) {
			...
			authorizedTwitterAccount.setNewFollowers(getNewFollowersCount(
					persona, authorizedTwitterAccount));
			authorizedTwitterAccount.setNewFriends(getNewFriendsCount(persona,
					authorizedTwitterAccount));
			authorizedTwitterAccount.setNewBlocking(getNewBlockingCount(
					persona, authorizedTwitterAccount));
	...
	private int getNewFriendsCount(PersonaDO persona,
			TwitterAccountDTO twAccount) {

		int newCount = 0;
		// Call twitter
		try {

			newCount = businessHelper.getTwitterPojo().getFollowingIds(
					twAccount).length;
	...
	private int getNewFollowersCount(PersonaDO persona,
		...
		// Call twitter
		try {
			newCount = businessHelper.getTwitterPojo().getFollowersIds(
					twAccount).length;
			// newCount =
			// TwitterServiceAdapter.get().getFollowersIds(twAccount).length;
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private int getNewBlockingCount(PersonaDO persona,
		...
		// Call twitter
		try {
			newCount = businessHelper.getTwitterPojo()
					.getBlockingIds(twAccount).length;

			// newCount =
			// TwitterServiceAdapter.get().getBlockingIds(twAccount).length;
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
